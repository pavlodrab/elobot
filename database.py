...
def merge_players(keep_id: int, drop_id: int) -> dict:
    ...
    Use case: an admin called ``/admin_setnick @oldhandle Nick`` for a
    player who later removed their public Telegram ``@username`` and
    interacted with the bot, creating a second row under the synthetic
    ``id_<telegram_id>`` username. ``merge_players`` reassigns every
    foreign key (matches / tournament_players / tournament_elo /
    match_goals / matches.reported_by) from ``drop_id`` to ``keep_id``,
    combines per-tournament stats where both rows are entered into the
    same tournament, copies missing metadata (``telegram_id``,
    ``game_nickname``, lifetime stats sums, max ELO) into ``keep``,
    and finally deletes the ``drop`` row.

    Both rows must exist and ``keep_id != drop_id``.

    Returns a dict with simple counters: ``{"matches_moved": int,
    "tp_overlap": int, "elo_overlap": int, "goals_moved": int}`` so the
    caller can show a friendly summary.
    """
    ...
    counters = {"matches_moved": 0, "tp_overlap": 0, "elo_overlap": 0,
                "goals_moved": 0}

    # ── 1. tournament_players (PK: tournament_id, player_id) ──────────────
    # Merge stats where both rows exist for the same tournament; reassign
    # the rest.
    rows = c.execute(
    ...
    overlap_tids = [r[0] if not isinstance(r, dict) else r["tournament_id"] for r in rows]
    for tid in overlap_tids:
        ...
        c.execute(
            ...
            "    eliminated   = MAX(eliminated, ?) "
            "WHERE tournament_id=? AND player_id=?",
            (
                int(d.get("group_points") or 0),
                int(d.get("group_gf")     or 0),
                int(d.get("group_ga")     or 0),
                int(d.get("group_wins")   or 0),
                int(d.get("group_draws")  or 0),
                int(d.get("group_losses") or 0),
                int(d.get("eliminated")   or 0),
                tid, keep_id,
            ),
        )
        c.execute(
    ...
    counters["tp_overlap"] = len(overlap_tids)
    c.execute(
        "UPDATE tournament_players SET player_id=? WHERE player_id=?",
        (keep_id, drop_id),
    )

    # ── 2. tournament_elo (PK: tournament_id, player_id) ──────────────────
    try:
        rows = c.execute(
            "SELECT tournament_id FROM tournament_elo "
            "WHERE player_id = ? AND tournament_id IN ("
            "    SELECT tournament_id FROM tournament_elo WHERE player_id = ?"
            ")",
            (drop_id, keep_id),
        ).fetchall()
        elo_overlap = [r[0] if not isinstance(r, dict) else r["tournament_id"] for r in rows]
        for tid in elo_overlap:
            d_row = c.execute(
            ...
            if d_row is None:
                continue
            d = dict(d_row)
            # Keep the higher ELO of the two (don't punish the merged
            # player) and sum game/win/loss/draw counters.
            c.execute(
                ...
                "WHERE tournament_id=? AND player_id=?",
                (
                    float(d.get("elo")           or 0.0),
                    int(d.get("games")           or 0),
                    int(d.get("wins")            or 0),
                    int(d.get("draws")           or 0),
                    int(d.get("losses")          or 0),
                    int(d.get("goals_for")       or 0),
                    int(d.get("goals_against")   or 0),
                    tid, keep_id,
                ),
            )
            c.execute(
        ...
        counters["elo_overlap"] = len(elo_overlap)
        c.execute(
            "UPDATE tournament_elo SET player_id=? WHERE player_id=?",
            (keep_id, drop_id),
        )
    except Exception:
        # tournament_elo is a newer table; tolerate older schemas.
        pass

    # ── 3. matches.player1_id / player2_id / reported_by ──────────────────
    c.execute(
        "UPDATE matches SET player1_id=? WHERE player1_id=?",
        (keep_id, drop_id),
    )
    counters["matches_moved"] += c.rowcount or 0
    c.execute(
        "UPDATE matches SET player2_id=? WHERE player2_id=?",
        (keep_id, drop_id),
    )
    counters["matches_moved"] += c.rowcount or 0
    # ``reported_by`` is sometimes a player.id and sometimes a
    # telegram_id depending on the call site. Best-effort: only rewrite
    # cases where it equals the dropped player.id (telegram_id range
    # rarely overlaps the small autoincrement ids used by players).
    c.execute(
        "UPDATE matches SET reported_by=? WHERE reported_by=?",
        (keep_id, drop_id),
    )
    ...
    try:
        ...
        counters["goals_moved"] = c.rowcount or 0
    except Exception:
        pass

    # ── 5. Merge metadata into ``keep`` ───────────────────────────────────
    updates: dict[str, object] = {}
    if not keep.get("telegram_id") and drop.get("telegram_id"):
        updates["telegram_id"] = drop["telegram_id"]
    if not (keep.get("game_nickname") or "").strip() and \
            (drop.get("game_nickname") or "").strip():
        updates["game_nickname"] = drop["game_nickname"]
    # If keep still has the synthetic ``id_<digits>`` username and
    # drop has a real ``@handle``, prefer the real handle on keep.
    keep_uname = (keep.get("username") or "").strip().lower()
    drop_uname = (drop.get("username") or "").strip().lower()
    import re as _re
    keep_synth = bool(_re.match(r"^id_\d+$", keep_uname))
    drop_synth = bool(_re.match(r"^id_\d+$", drop_uname))
    if keep_synth and drop_uname and not drop_synth:
        updates["username"] = drop_uname
    # Lifetime stats sums.
    for f in (
        "goals_scored", "goals_conceded", "assists",
        "wins", "losses", "draws", "clean_sheets",
    ):
        updates[f] = int(keep.get(f) or 0) + int(drop.get(f) or 0)
    # Max of best-streak; reset current win_streak (ambiguous after merge).
    updates["best_streak"] = max(
        int(keep.get("best_streak") or 0),
        int(drop.get("best_streak") or 0),
    )
    # ELO: keep the higher of the two so the merge never silently demotes
    # the player. Same for the per-type pools.
    for f in ("elo", "elo_vsa", "elo_ri"):
        if (drop.get(f) or 0) > (keep.get(f) or 0):
            updates[f] = drop[f]
    # Ban status: keep the more restrictive ban.
    if drop.get("banned_until") and not keep.get("banned_until"):
        updates["banned_until"] = drop["banned_until"]
        if drop.get("banned_reason"):
            updates["banned_reason"] = drop["banned_reason"]

    if updates:
        # Clear any UNIQUE-constrained values on the drop row first so
        # the keep update can adopt them without conflict.
        if "telegram_id" in updates:
            c.execute(
                "UPDATE players SET telegram_id=NULL WHERE id=?",
                (drop_id,),
            )
        if "username" in updates:
            # Replace drop's username with a free, throw-away placeholder
            # so the UNIQUE(username) constraint doesn't block reusing
            # the real handle on ``keep`` before drop is deleted.
            c.execute(
                "UPDATE players SET username=? WHERE id=?",
                (f"_merged_{drop_id}", drop_id),
            )
        sets = ", ".join(f"{k}=?" for k in updates)
        vals = list(updates.values()) + [keep_id]
...
def get_player_by_telegram_id(tid: int | None):
    """Find a player row by their numeric Telegram user id (or None)."""
    if tid is None:
        return None
    conn = get_conn()
    p = conn.execute(
...
def get_player_by_game_nickname(game_nick: str):
    """Find player by case-insensitive in-game nickname."""
    if not game_nick:
        return None
    conn = get_conn()
    p = conn.execute(
...
def find_players_by_fuzzy_game_nickname(query: str, threshold: float = 0.55, limit: int = 5):
    """
    Fuzzy search through all registered game_nicknames.
    Returns list of (player_dict, ratio) sorted by best match first.
    Used when OCR mis-reads a few characters of the in-game nickname.
    """
    ...
    conn = get_conn()
    rows = conn.execute(
        "SELECT * FROM players WHERE game_nickname IS NOT NULL AND game_nickname != ''"
    ).fetchall()
    conn.close()
    ...
    for r in rows:
        ...
        if not nick:
            continue
        ratio = SequenceMatcher(None, query_l, nick).ratio()
        # Boost matches where one is a substring of the other (handles partial OCR).
        if query_l in nick or nick in query_l:
...
def get_all_players_by_elo_field(field: str = "elo"):
    """Return players sorted by `elo`, `elo_vsa`, or `elo_ri`."""
    if field not in ("elo", "elo_vsa", "elo_ri"):
        field = "elo"
    conn = get_conn()
    rows = conn.execute(f"SELECT * FROM players ORDER BY {field} DESC").fetchall()
    conn.close()
